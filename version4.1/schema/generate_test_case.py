...
RULES_FILE = "conditional_rules.json"
EXAMPLE_FILE = "examples/simple_flow_example.csv"
...
def save_cost_log(session: SessionStats, output_dir: Path):
    """Save cost log to JSON file."""
    log_file = output_dir / "cost_log.json"
...
def main():
    ...
    try:
        schema = load_json(script_dir / SCHEMA_FILE)
        rules = load_json(script_dir / RULES_FILE)
        example_csv = load_file(script_dir / EXAMPLE_FILE)
    ...
    if args.interactive:
        ...
        while True:
            ...
            try:
                ...
                if args.validate:
                    print("\nValidating...")
                    os.system(f"python {script_dir}/validator.py {filepath} {script_dir/SCHEMA_FILE} {script_dir/RULES_FILE}")
        ...
        if session.total_calls > 0:
            session.print_summary()
            save_cost_log(session, script_dir / OUTPUT_DIR)
